...
class PresentaionLayer
{
            ...
            switch (userInput)
            {
                case 1: //roze lakahat azmana
                        ...
                        if (firstime) //lakoah rishon she poteah database mazmin kodem gviim o kufsa
                        {
                            ...
                            userInput = viewer(orderId, ballsID, decorateID, icecreamID, coonsID, ownerID, VUseId);
    ...
    //int orderId = 0, ballsID = 0, decorateID = 0, icecreamID = 0, coonsID = 0, ownerID = 0, VUseId = 0;
    static int viewer(int orderId, int ballsID, int decorateID, int icecreamID, int coonsID, int ownerID, int VUseId)
    {
        ...
        b = new Balls(type_of_ball, type_of_ball.Length, 0);//neadken ereh aharon price kshenaavor et coons
                                                            //gam im hu rak mazmin kadurim ve over halaa ani roze et hareshuma hazot 
        c = new Coons(icecreamID,null, 0);
        de = new Decorate(null, 0);
        o = new Owner(ownerID, null, null, null);
        I = new Icecream(icecreamID, ballsID, coonsID, decorateID, 0); //maby i should add coonsID field
        v = new VUse(ownerID, icecreamID);
        or = new Order(icecreamID, 0, day, day, 0);
        int dada = 5;
        int complete = 0;
        int pay = 0;
        //updating all evin if he going to exit iam going to update 
        //Logic.createSalle(or);
        update_all(b, c, de, o, I, v, or);
        I.setPrice(0);
        //c.set_id(icecreamID);
        //  Logic.createBalls(b);
        ...
        int userInput = -5;                                  //type_ball ze maarah        
        do //price will be for cons object beyaham im hakadurim
        {
            ...
            userInput = Int32.Parse(Console.ReadLine()); //im hu yoze eshli object kodmim shmurim
       ...
       // I.setPrice(price);
        if (first[0] == 1) //avarnu sinun rishoni kaet kishut
        {
           ...
           // BusinessLogic.Logic.update(or, icecreamID);
            ...
            //  Logic.createCons(c);
            //d = new Decorate(decorateID, null, 0);
